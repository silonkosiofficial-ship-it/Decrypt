package p258z9;

/* JADX INFO: loaded from: classes2.dex */
class Q extends p258z9.AbstractC7392e {

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    private final java.util.Map f57689g;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public Q(p249y9.AbstractC7358b abstractC7358b, p237x7.l lVar) {
        super(abstractC7358b, lVar, null);
        p247y7.AbstractC7350t.f(abstractC7358b, "json");
        p247y7.AbstractC7350t.f(lVar, "nodeConsumer");
        this.f57689g = new java.util.LinkedHashMap();
    }

    @Override // p258z9.AbstractC7392e
    public p249y9.i q0() {
        return new p249y9.D(this.f57689g);
    }

    @Override // p258z9.AbstractC7392e
    public void v0(java.lang.String str, p249y9.i iVar) {
        p247y7.AbstractC7350t.f(str, "key");
        p247y7.AbstractC7350t.f(iVar, "element");
        this.f57689g.put(str, iVar);
    }

    protected final java.util.Map w0() {
        return this.f57689g;
    }

    @Override // p239x9.C0, p229w9.f
    public void x(p219v9.f fVar, int i6, p199t9.e eVar, java.lang.Object obj) {
        p247y7.AbstractC7350t.f(fVar, "descriptor");
        p247y7.AbstractC7350t.f(eVar, "serializer");
        if (obj != null || this.f57748d.j()) {
            super.x(fVar, i6, eVar, obj);
        }
    }
}
